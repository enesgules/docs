...
Eviction keeps a cache inside it's size limit, but it does not protect the
database from a client that writes to fast. To cap the write rate per client,
see [Rate Limit Your API](/redis/howto/ratelimit-api) and the
