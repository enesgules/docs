...
Eviction keeps a cache inside its size limit, but it does not protect the
database from a client that writes too fast. To cap the write rate per client,
see [Rate Limit Your API](/redis/howto/rate-limit-api) and the
[eviction policies](/redis/features/eviction#eviction-policies) we support.
